  ...
  <div class="prom-advantage">
    <div class="advantage-banner-part">
      <swiper ref="mySwiper" :options="swiperOption">
        <swiper-slide class="news-banner carousel-item">
          <img src="~images/prom/news-mobile-banner.jpg"/>
          <div class="text-info">
            <div class="pc">
              恭喜<span class="cl-red">{{winUser}}</span>暂时以<span class="cl-red">{{winAmount}}</span>万赠币荣获榜首
            </div>
            <div class="mobile">
              恭喜<span class="cl-red">{{winUser}}</span>
              <br/>
              暂时以<span class="cl-red">{{winAmount}}</span>万赠币荣获榜首
            </div>
          </div>
        </swiper-slide>
        <swiper-slide class="index-banner carousel-item">
          <img src="~images/prom/index-mobile-banner.jpg"/>
        </swiper-slide>
        ...
        <div class="row">
          <div class="advcont-nav cfx" v-show="!isOfficialWeb">
            <div class="col-21" v-for="(item,i) in advantageNav" :key="i">
              <a href="javascript:void(0);" @click="advantageType=item.value"
                 :class="[{active:advantageType==item.value},item.value]">
...
<script>
  ...
  export default {
    ...
    methods: {
      getWinInfo(){
        awardRankings({}).then(res => {
          if(res.code === 10000){
            this.winUser = res.data[0].nickname
            this.winAmount = res.data[0].awardTotal
          }
        })
      }
    },
...
<style lang="scss">
  .advantage-banner-part {
    ...
    .news-banner{
      background: url("~images/prom/news-banner.jpg") no-repeat center center;
      background-size: cover;
    }
    .index-banner{
      background: url("~images/prom/index-banner.jpg") no-repeat center center;
      background-size: cover;
    }
    .title {
      display: none;
